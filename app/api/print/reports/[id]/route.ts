...
export async function GET(
  ...
  // ── 1. Auth check — requester must be an authenticated partner user ────────
  ...
  if (!profile?.partner_id) return new NextResponse('Forbidden', { status: 403 })

  // ── 2. Fetch report — verify ownership + approved/paid status ─────────────
  const admin = createAdminClient()
  const { data: report } = await admin
    .from('monthly_reports')
    .select('reporting_month, reporting_year, status, branches(name, partner_id)')
    .eq('id', id)
    .single()
  ...
  if (branch?.partner_id !== profile.partner_id)
    return new NextResponse('Forbidden', { status: 403 })
  if (!['approved', 'paid'].includes(report.status))
    return new NextResponse('Forbidden', { status: 403 })

  const branchName = branch?.name ?? 'Report'
  ...
  const printUrl = `${origin}/print/reports/${id}`
  ...
  try {
    ...
    console.error('[PDF reports] generation failed:', err)
